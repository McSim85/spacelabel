...
| 10.3 | **No PyPI for now — distribution is pipx from GitHub Releases.** `publish.yml` attaches the built wheel + sdist to each GitHub Release; users install via `pipx install <wheel-url>` or `pipx install git+<repo>@<tag>`. PyPI can be added later with zero workflow changes. | Personal tool; PyPI registration + OIDC setup is overhead not yet warranted. Releasing to GitHub Releases is sufficient for the pipx-first install path. | high |
...
| 10.5 | **Homebrew formula lives IN this repo at `Formula/spacelabel.rb`, not in a separate `homebrew-spacelabel` repo.** Tap via: `brew tap McSim85/spacelabel https://github.com/McSim85/spacelabel`. The `publish.yml` formula-update job uses `GITHUB_TOKEN` to PR the bump (no extra PAT). Guarded by `FORMULA_IN_REPO=true` variable until the formula is committed. | No extra repo to maintain, no PAT secret. Homebrew supports tapping any public GitHub repo with a full URL. The formula stub is in `packaging/homebrew/spacelabel.rb`; it moves to `Formula/spacelabel.rb` when the repo goes public. | high |
...
**Open questions:**
- Homebrew formula resource sha256s must be filled in via `brew update-python-resources` before the tap is usable (one-time manual step; documented in `.github/SETTINGS.md`).
- The release-please PR uses `GITHUB_TOKEN`; if branch protection requiring human review is enabled, Max must merge it manually — which is the correct flow.
