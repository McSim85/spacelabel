...
**PASS — no blocking defects.** Every make-or-break behavior is verified on the reference Mac (macOS 26.5.1, M3 Pro, dual display). The distribution pivot to a signed `.app` via Homebrew cask shipped (PR #32) and is verified end-to-end. All gaps found are **non-blocking backlog**; the experimental wallpaper mode was subsequently **removed** (2026-06-25, DECISIONS §7 — the R+S findings below drove that); three checks are deferred to a natural restart/hardware window.
...
- **Wallpaper (REMOVED 2026-06-25, DECISIONS §7):** **R** Dynamic/Shuffle clobbering (static-only impossible) · **S** can't capture the per-Space base (`desktopImageURL` returns the default). These findings were left UNVERIFIED by design and ultimately drove removing the mode entirely.
- **UX/polish:** **T** Prefs/color-picker window placement + re-surface · **U** Prefs inline-edit (Cmd+V dead, no live-revert on clear) · **W** menu-bar OFF shows empty quadrant not `square.dashed` · **P/Q** per-display overlay on/off + suppress on unlabeled · **K** CLI prog_name `launcher.py` · **M** `status --help` markup leak (+test) · **N** colorize `status` output · **Y** `NO_COLOR` not honored by table color. — **✅ K+M+N+Y FIXED (2026-06-25, PR #59 `fix/cli-polish`):** `main(prog_name=…)` forwarded in launcher; eight docstrings cleaned; `status` green/yellow; `_color_arg()` (`"NO_COLOR" in env`, presence-not-value) wired into `_echo_table` + status; 454 tests pass including parametrized markup guard + color/NO_COLOR suite.
- **Already-tracked tasks:** `todo/remove-pipx.md` (**✅ done 2026-06-25, `refactor/remove-pipx`:** `_canonical_shim` + pipx tier in `_resolve_install_shim` removed; `uv` dev path kept; all code/tests/docs/packaging/CI updated; 455 tests pass; DECISIONS §6.6/§6.8 updated), `todo/uninstall-purge.md` (the `--purge` feature + its 1B.5–1B.9 acceptance rows).
...
Still need your hands/eyes: (3) display modes (menu-bar / HUD / overlay) on the rotated 2160×3840 + 4K, (4) menu-bar/prefs rename live-reload, (5) reboot persistence [deferred to a natural restart — snippet below], (6) generality spot-check (detach 4K / change res-orientation / add-remove Space). [Wallpaper-revert check dropped — mode removed, DECISIONS §7.]
...
> **Update (2026-06-25):** the wallpaper findings below (items R+S) drove **removing wallpaper mode entirely** (DECISIONS §7). The wallpaper notes are retained as the dated evidence for that decision; the mode no longer ships.
