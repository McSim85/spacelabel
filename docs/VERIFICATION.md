...
- **[P2, found this pass] cask-bump asset download was unauthenticated** — `update-cask` hashed the release zip over an anonymous `curl` URL, which 404s for this **private** repo and would stall the bump after upload. Switched to `gh release download` (uses `GH_TOKEN`) → `sha256sum`. The release workflow itself stays untested until the first real release (no push this session).
...
- **[P1 — DOC, infra decision for Max] cask points at a private-repo release URL** — `brew` fetches the asset **anonymously**, so it 404s while the repo is private. No code change makes a private GitHub asset anonymously fetchable; the cask works only once the repo is **public** (the planned MIT release) or the asset is on a public host. Documented in the cask `url` comment + README. **→ Max's call.**
