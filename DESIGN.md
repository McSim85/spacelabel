...
Phase-1 research was conducted with a 7-track parallel workflow plus two adversarial verifiers and a completeness critic; the riskiest two claims (Python→CGS reads on Tahoe, and a menu-bar agent under launchd) were **empirically confirmed on the reference machine** (macOS 26.5.1, arm64, SIP enabled). Where a claim is design-only / inferred it is marked and pushed to the Phase-6 probe.
...
All agent windows run under `NSApplicationActivationPolicyAccessory` (set in code at `applicationDidFinishLaunching_`; no Dock icon, no app menu, no `LSUIElement` plist needed for dev runs — declared in `Info.plist` in the bundle). HUD and overlay must **never steal focus**.

### 6.1 Menu-bar item (primary) — raw `NSStatusItem` (no `rumps`)
`NSStatusBar.systemStatusBar().statusItemWithLength_(NSVariableStatusItemLength)`; set `button().setTitle_(label)`; attach an `NSMenu` (Preferences…, Quit). Update the title from the debounced space-change callback. **`rumps` is rejected** — it is a thin wrapper over these same ~15 calls, would take over `NSApplication`/run-loop ownership (colliding with our delegate + workspace observer), and is poorly packaged (sdist-only, declares no PyObjC dep). See `DECISIONS.md`.
...
### 9.1 Homebrew cask (distribution) — signed `.app` bundle
- One package, one `console_scripts` entry point `spacelabel = "spacelabel.cli:main"`. The cask ships a self-contained `spacelabel.app` (embeds Python.framework + PyObjC + click); its `binary` stanza puts the same exe on PATH as the CLI (DECISIONS §6.8).
- **Dependencies:** `pyobjc-core`, `pyobjc-framework-Cocoa`, `pyobjc-framework-Quartz`, `pyobjc-framework-CoreText`, `click`. SkyLight/CGS is loaded at runtime — **never** a PyPI dep. `rumps` is **not** a dependency (sdist-only, declares no deps, stale).
- **Library validation:** the bundle is **ad-hoc-signed, non-hardened** (verified `flags=0x2(adhoc)`, no `0x10000`), so LV is not enforced and `ctypes`/PyObjC can dlopen Apple-signed private frameworks with **no entitlement and no SIP change**. (A Developer-ID hardened+notarized bundle would re-engage LV — deferred, DECISIONS §6.9.)
...
<key>ProgramArguments</key>       <array>
  <string>/Applications/spacelabel.app/Contents/MacOS/spacelabel</string>   <!-- ABSOLUTE: launchd does not expand ~ or read $PATH -->
  <string>agent</string>
</array>
...
- Interpreter pinning caveat: the dev `uv` venv is pinned to Homebrew `python@3.14`; after a `brew upgrade python` minor bump, recreate it (`uv venv && uv pip install -e '.[dev]'`). The cask bundles its own interpreter — unaffected.

### 9.3 uv (dev)
`uv venv` → `uv pip install -e '.[dev]'` (ruff, mypy) → `uv run ruff check .` / `uv run mypy spacelabel` / `uv run spacelabel agent --debug`. uv uses `./.venv`. For a LaunchAgent test, `spacelabel install` falls back to the console script beside the interpreter (`.venv/bin/spacelabel`) — a durable dev path, never a transient runner path (DECISIONS §9.1).
