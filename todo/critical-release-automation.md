...
> - **DEFERRED build optimisation (Max, 2026-06-23):** today a release is **two PRs** — release-please (version+CHANGELOG) then the `update-cask` follow-up PR (sha256, uncomputable until the asset is built). To make it **one human merge**, give `update-cask` auto-merge (`gh pr merge --auto --squash`) + enable "Allow auto-merge" in repo Settings (keeps the real sha256 + branch protection). Alternatives: direct-commit the bump to main (needs a bot branch-protection bypass) or `sha256 :no_check` + release-please `extra-files` (loses the integrity check). **Deferred for now** — works as-is, just merge both PRs per release.
