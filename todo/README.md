...
| Detect a **stale Accessibility grant** (ad-hoc cdhash rotates per build) + better guidance instead of "enable it" | Non-critical | [improvements.md](improvements.md) item L | next | bundled | **open** (Max, 2026-06-23) — live: named "spacelabel" entry enabled but stale → click-to-switch still blocked; durable fix = Developer-ID notarization (item E) |
