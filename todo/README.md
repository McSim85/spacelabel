...
| **Remove pipx entirely** — cask is the sole supported distribution; strip the deprecated pipx path from code/tests/docs/packaging/CI | **Critical** | [remove-pipx.md](remove-pipx.md) | next | 1 session · Sonnet 4.6 · medium | **✅ done** (2026-06-25, `refactor/remove-pipx`) — `_canonical_shim` + pipx tier removed; `uv` dev path kept; all docs + tests updated; DECISIONS §6.6/§6.8 updated. |
| Click-to-switch pills — implement the opt-in `CGEventPost` switch path | **Critical** | [critical-click-to-switch.md](critical-click-to-switch.md) | v0.2/v0.3 | 1 session · Opus 4.8 · high | done — Phase-6 found Accessibility broken without the signed bundle; fix = signed `.app` cask (phase-6-blockers Tier 1, done 2026-06-22) |
...
| Hover tooltip on menu-bar pills (full Space name) | Non-critical | [improvements.md](improvements.md) item AC | v1.1 | bundled | **✅ done** (2026-06-25, `feat/pill-tooltip`) — hardware-pending |
| Homebrew agent-path fix — `_resolve_install_shim` resolves the **app-bundle** exe | **Critical** | [critical-release-automation.md](critical-release-automation.md) → [phase-6-blockers.md](phase-6-blockers.md) Tier 1 step 5 | next | folded into phase-6-blockers | **done** (2026-06-22; `_enclosing_app_exe` resolves the bundle exe; pipx fallback removed in `refactor/remove-pipx`) |
...
| [remove-pipx.md](remove-pipx.md) | — (deprecated pipx path) | medium · Sonnet 4.6 | **B** | ✅ done (2026-06-25) |
...
- **🔀 Distribution pivot (Max, 2026-06-22) + pipx removal (2026-06-25):** moved from **pipx-only** to a **signed `.app` shipped via a Homebrew cask** (reverses #30, DECISIONS §6.8), then **removed the deprecated pipx path entirely** (`refactor/remove-pipx`, DECISIONS §6.6/§6.8). The formal `DECISIONS.md` update (§6/§2.7/§6.3) landed with `feat/signed-app-cask`; §6.6/§6.8 updated with `refactor/remove-pipx`.
