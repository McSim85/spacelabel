...
**✅ DONE (2026-06-24, same PR as O+V).** Chose option 2 (make it switchable): the default Space is now a switch target via its session `id64` (the pill carries `id64`; `_handle_click_at_x` routes a `uuid`-or-`id64` pill to the switch handler; `_on_pill_clicked` resolves by `id64` when `uuid==""`), still gated to the active display (item O). A pill with neither a uuid nor an id64 still opens the menu. Recorded the §9.5 update (unlabelable Spaces are now switchable by id64) in DECISIONS.md.
