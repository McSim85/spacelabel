...
- **1B.8** — `spacelabel uninstall --purge --yes`: removes the agent, then deletes the four targets; each delete independent/best-effort; partial failure prints what couldn't be removed and exits 1. **Never** touches the WallpaperAgent store/container plists, the pipx venv, or the `~/.local/bin/spacelabel` shim.
