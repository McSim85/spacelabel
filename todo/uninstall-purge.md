...
- **`--purge` (= `apt purge`):** after agent removal, delete **only** spacelabel-owned, named targets:
  ...
  **Never touch:** the pipx venv, the `~/.local/bin/spacelabel` shim, or the `.zshrc` `fpath` line (mention it in output for manual removal).
...
- **1B.8** — `spacelabel uninstall --purge --yes`: removes the agent, then deletes the four targets; each delete independent/best-effort; partial failure prints what couldn't be removed and exits 1. **Never** touches the pipx venv or the `~/.local/bin/spacelabel` shim.
